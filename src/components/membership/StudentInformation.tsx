...
import { ImageUploader, UploaderDragerButton } from "../controls";
...
export const StudentInformation: FC<IStudentInformation> = () => {
  ...
  const [toggleStudentIdImage, setToggleStudentIdImage] = useState(false);
  const {
    control,
    watch,
    formState: { errors },
  ...
  const studentIdFetch = watch("studentIdFetch");
  ...
  const existStudentIdImage = watch("studentIdImage");
    ...
    <div>
      ...
      <Row className="bg-white py-6 px-16 border-t-4 border-library-primary rounded-b-lg mt-5">
        ...
        <Col
          ...
          xl={{ span: 12 }}
        >
          {router?.pathname === "/user/change-membership/upgrade-student" ? (
            ""
          ) : (
          ...
          <div className="studentId mt-4">
            <label htmlFor="studentIdUpload" className="text-sm text-gray-700">
              {localeString(language, "studentIdImage")}
              <span className="text-red-700">*</span>
            </label>
            <div className="mt-3">
              {mode == "edit" ? (
                <>
                  {!existStudentIdImage?.fileList && (
                    <>
                      <Image src={studentIdFetch || ""} width={80} alt="" />
                      <Button
                        className="mb-4 flex"
                        onClick={() =>
                          setToggleStudentIdImage(!toggleStudentIdImage)
                        }
                      >
                        {localeString(language, "change")}
                      </Button>
                  ...
                  {toggleStudentIdImage && (
                    <ImageUploader
                      name="studentIdImage"
                      istext={"Upload Student ID Image"}
                      buttonText={"Upload Student ID Image"}
                      isButtonFullScreen
                      control={control}
                      errors={errors}
                      acceptFileFormat={acceptFileFormat}
